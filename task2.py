...
def son(baskets, support, output_filepath):
    ...
    # print('phase_1 ', frequent_itemsets)
...
def execute_task2():
    ...
    if len(sys.argv) > 3:
        ...
        # input_filepath = './ta_feng_short.csv'
    ...
    # create columns
    baskets = data_rdd.map(lambda s: (s[0][1:-1] + "-" + str(int(s[1][1:-1])), str(int(s[5][1:-1]))))
    # baskets = data_rdd.map(lambda s: (s[0][1:-1][:-4] + s[0][1:-1][-2:] + '-' + str(int(s[1][1:-1])), str(int(s[5][1:-1]))))
    # create working dataset
    baskets = baskets\
        .groupByKey()\
        .mapValues(list)\
        .map(lambda s: list(s[1]))\
        .filter(lambda s: len(s) > filter_threshold).map(set).persist()
    # print('baskets: ', baskets.collect())
